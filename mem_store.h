...
class MemStore {
 public:
  ...
  virtual void OnDirectWrite(uint64_t phy_addr, uint64_t mach_addr) = 0;
  // When no entry is hit but a mapping is evicted
  virtual void OnWriteBack(uint64_t phy_addr, uint64_t mach_addr) = 0;

  // When a dirty entry is hit
  virtual void OnOverwrite(uint64_t phy_addr, uint64_t mach_addr) = 0;
  // When a clean entry is hit
  virtual void OnShrink(uint64_t phy_addr, uint64_t mach_addr) = 0;
 ...
 public:
  ...
  void OnWriteBack(uint64_t phy_addr, uint64_t mach_addr) {
    ...
    std::cout << "[Info] MemStore writes back from "
        << mach_addr << " to " << phy_addr << std::endl;
    std::cout << std::dec;
  }
  ...
  void OnShrink(uint64_t phy_addr, uint64_t mach_addr) {
    assert(phy_addr != mach_addr);
    std::cout << std::hex;
