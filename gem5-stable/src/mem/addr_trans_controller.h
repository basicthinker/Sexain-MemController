...
enum Control
{
    REG_WRITE, NEW_EPOCH, WAIT_CKPT,
};

class AddrTransController
{
  public:
    AddrTransController(uint64_t dram_size, Addr phy_limit, int att_len,
            int block_bits, int page_bits, MemStore* ms);

    Addr LoadAddr(Addr phy_addr, Profiler& pf);
    Control Probe(Addr phy_addr);
    Addr StoreAddr(Addr phy_addr, int size, Profiler& pf);

    void BeginCheckpointing(Profiler& pf);
    void FinishCheckpointing();
    void MigratePages(Profiler& pf, double dr = 0.33, double wr = 0.67);

    uint64_t Size() const;
    int blockSize() const
    {
        return att_.unitSize();
    }
    int page_size() const
    {
        return migrator_.page_size();
    }
    int att_length() const
    {
        return att_.length();
    }
    bool in_checkpointing() const
    {
        return in_checkpointing_;
    }
    const MigrationController& migrator() const
    {
        return migrator_;
    }

    uint64_t pages_to_dram() const
    {
        return pages_to_dram_;
    }
    uint64_t pages_to_nvm() const
    {
        return pages_to_nvm_;
    }

    bool IsDRAM(Addr phy_addr, Profiler& pf);
#ifdef MEMCK
...
private:
    ...
    public:
        ...
        void Visit(int i);
        int num_flushed_entries() const
        {
            return num_flushed_entries_;
        }
    private:
    ...
    {
    public:
        LoanRevoker(AddrTransController* atc, Profiler& pf)
                : atc_(atc), pf_(pf)
        {
        }
        void Visit(int i);
...
inline void AddrTransController::CopyBlockIntra(Addr dest_addr, Addr src_addr,
        Profiler& pf)
{
...
inline void AddrTransController::SwapBlock(Addr direct_addr, Addr hw_addr,
        Profiler& pf)
{
